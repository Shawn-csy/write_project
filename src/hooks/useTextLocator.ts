...
export function useTextLocator(rawScript: string) {
  const readerHighlightRef = useRef<Element | null>(null);
  const readerHighlightTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  const handleLocateText = useCallback((text: string, lineNumber?: number) => {
    const container = contentScrollRef.current;
    if (!container || !rawScript) return;

    const targetLine = Number.isFinite(lineNumber) ? (lineNumber as number) : null;
    if (targetLine) {
