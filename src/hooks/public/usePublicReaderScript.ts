        ...
        if (script) {
          ...
          const publicProjection = buildPublicReaderProjection({
            ...
            activity: {
              ...
              bannerCrop: null,
              content: String(script.activityContent || "").trim(),
              demoUrl: (() => { const links = ensureList(script.activityDemoLinks); return String((links[0] as { url?: string })?.url || "").trim(); })(),
              demoLinks: ensureList(script.activityDemoLinks),
              workUrl: String(script.activityWorkUrl || "").trim(),
            },
