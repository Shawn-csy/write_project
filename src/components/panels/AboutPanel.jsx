        ...
        <div className="mt-8 pt-8 border-t border-border/60">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 h-auto lg:h-[500px] border border-border/60 rounded-lg overflow-hidden">
                ...
                <div className="bg-muted flex flex-col overflow-hidden border-b lg:border-b-0 lg:border-r border-border/60">
                    <div className="p-2 border-b border-border/40 bg-muted/50 text-xs text-muted-foreground font-bold uppercase tracking-wider shrink-0">{sourceLabel}</div>
                    <textarea 
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="rounded-lg border bg-card p-4 text-card-foreground shadow-sm">
          <div className="mb-2 flex items-center gap-2">
            ...
            <h3 className="font-semibold">{t("aboutPanel.coreFeatures")}</h3>
          </div>
          <ul className="space-y-2 text-sm text-muted-foreground">
            <li className="flex items-start gap-2">
              <span className="mt-1 block h-1.5 w-1.5 shrink-0 rounded-full bg-primary/60"></span>
              <span>{t("aboutPanel.feature1")}</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="mt-1 block h-1.5 w-1.5 shrink-0 rounded-full bg-primary/60"></span>
              <span>{t("aboutPanel.feature2")}</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="mt-1 block h-1.5 w-1.5 shrink-0 rounded-full bg-primary/60"></span>
              <span>{t("aboutPanel.feature3")}</span>
            </li>
          </ul>
        </div>
        ...
        <div className="rounded-lg border bg-card p-4 text-card-foreground shadow-sm flex flex-col justify-between">
          <div>
            <div className="mb-2 flex items-center gap-2">
              ...
              <h3 className="font-semibold">{t("aboutPanel.highlights")}</h3>
            </div>
            <ul className="mt-1 text-sm text-muted-foreground list-disc list-inside space-y-1">
              <li>{t("aboutPanel.highlight1")}</li>
              <li>{t("aboutPanel.highlight2")}</li>
              <li>{t("aboutPanel.highlight3")}</li>
              <li>{t("aboutPanel.highlight4")}</li>
            </ul>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex-1 min-h-0 overflow-hidden border border-border bg-background/60 rounded-xl shadow-sm flex flex-col">
       <div className="p-4 border-b border-border/60 bg-muted/20 flex-shrink-0 flex items-center gap-3">
          ...
          <h2 className="text-xl font-bold tracking-tight">{t("aboutPanel.systemInfoGuide")}</h2>
       </div>
         ...
         <Tabs defaultValue="about" className="h-full flex flex-col">
            <div className="px-4 pt-2 shrink-0">
              <TabsList className="grid w-full max-w-[400px] grid-cols-2">
                <TabsTrigger value="about">{t("aboutPanel.tabAbout")}</TabsTrigger>
                <TabsTrigger value="markers">{t("aboutPanel.tabMarkers")}</TabsTrigger>
              </TabsList>
            </div>
            ...
            <TabsContent value="markers" className="flex-1 overflow-y-auto min-h-0 p-6 pt-2">
                <div className="max-w-4xl mx-auto space-y-6 pb-12">
                   <div className="prose prose-sm dark:prose-invert max-w-none">
                      <h3>{t("aboutPanel.markerSettings")}</h3>
                      <MarkerSettingsGuide />

                      <DemoEditor 
                        title={t("aboutPanel.markerTry")} 
                        description={t("aboutPanel.demoDescription")}
                        sourceLabel={t("aboutPanel.sourceInput")}
                        previewLabel={t("aboutPanel.previewResult")}
