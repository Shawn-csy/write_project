...
export function ScriptMetadataPublishSection({
  ...
  const {
    ...
    licenseNotify, setLicenseNotify, publishNewTerm, setPublishNewTerm, addLicenseSpecialTerm,
    licenseSpecialTerms, removeLicenseSpecialTerm,
  } = useLicenseContext();
            ...
            {(licenseSpecialTerms || []).length > 0 && (
              <div className="space-y-2">
                {licenseSpecialTerms.map((term, idx) => (
                  ...
                  >
                    <span className="text-sm">{String(term || "")}</span>
