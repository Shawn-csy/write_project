...
import type { GoogleDocsBlock } from "./googleDocsExportModel";
...
const contactRows = (rawContact: unknown): ExportMetadataField[] => {
  ...
  if (value && typeof value === "object" && !Array.isArray(value)) {
    return Object.entries(value as Record<string, unknown>)
      ...
      .map(([key, val]) => ({ key: "contact", label: "聯絡", value: `${key}: ${val}` }));
  }
  if (Array.isArray(value)) {
    return value.map((item) => normalize(item)).filter(Boolean).map((item) => ({ key: "contact", label: "聯絡", value: item }));
  }
  return normalize(value)
    .split(/\r?\n|\/|\||；|;|，|,/)
    .map((item) => item.trim())
    .filter(Boolean)
    .map((item) => ({ key: "contact", label: "聯絡", value: item }));
};
...
export const buildExportMetadata = (source: ExportMetadataSource | null | undefined, fallbackTitle = "Script"): ExportMetadata => {
  ...
  const fields = [
    { key: "title", label: "標題", value: normalize(source?.title) || fallbackTitle || "Script" },
    synopsis ? { key: "synopsis", label: "簡介", value: synopsis } : null,
    organizationName ? { key: "organization", label: "組織", value: organizationName } : null,
    authorName ? { key: "author", label: "作者", value: authorName } : null,
    draftDate ? { key: "date", label: "日期", value: draftDate } : null,
    seriesName ? { key: "series", label: "系列", value: `${seriesName}${seriesOrder ? ` #${seriesOrder}` : ""}` } : null,
    tags.length ? { key: "tags", label: "標籤", value: tags.join("、") } : null,
    ...contactRows(meta.contact),
    ...formatLicense(commercial, derivative, notify).map((value) => ({ key: "license", label: "授權", value })),
