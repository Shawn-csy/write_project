...
export interface PresentationGroupedRow {
  line: number;
...
const rowHasDialogueEvent = (row: PresentationGroupedRow, tracks: TrackConfig[]): boolean =>
  tracks.some((track) => (
...
const pushEvent = (row: PresentationGroupedRow, trackId: string, event: ScriptEvent) => {
...
export const buildGroupedRows = (
  ...
  const rows: PresentationGroupedRow[] = [];
  const rowByLine = new Map<number, PresentationGroupedRow>();
  const rangeRowsByKey = new Map<string, PresentationGroupedRow[]>();
  ...
  events.forEach((item) => {
    const line = Number.isFinite(item.event.lineSpan?.start)
      ...
      : 1;
    let row: PresentationGroupedRow | undefined;
